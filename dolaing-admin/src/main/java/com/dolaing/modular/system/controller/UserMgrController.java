...
import com.dolaing.core.common.constant.dictmap.UserDict;
...
import com.dolaing.modular.system.factory.UserFactory;
import com.dolaing.modular.system.model.User;
import com.dolaing.modular.system.service.IUserService;
import com.dolaing.modular.system.transfer.UserDto;
import com.dolaing.modular.system.warpper.UserWarpper;
...
public class UserMgrController extends BaseController {
    ...
    public Object changePwd(@RequestParam String oldPwd, @RequestParam String newPwd, @RequestParam String rePwd) {
        if (!newPwd.equals(rePwd)) {
            throw new DolaingException(BizExceptionEnum.TWO_PWD_NOT_MATCH);
        }
        Integer userId = ShiroKit.getUser().getId();
        ...
        if (user.getPassword().equals(oldMd5)) {
            ...
            throw new DolaingException(BizExceptionEnum.OLD_PWD_NOT_RIGHT);
        }
    }
    ...
    @Permission
    @ResponseBody
    public Object list(@RequestParam(required = false) String name, @RequestParam(required = false) String beginTime, @RequestParam(required = false) String endTime, @RequestParam(required = false) String type) {
        if (ShiroKit.isAdmin()) {
            List<Map<String, Object>> users = userService.selectUsers(null, name, beginTime, endTime, type);
            return new UserWarpper(users).warp();
        } else {
            DataScope dataScope = new DataScope(null);
            List<Map<String, Object>> users = userService.selectUsers(dataScope, name, beginTime, endTime, type);
            return new UserWarpper(users).warp();
        }
    }
    ...
    public Tip add(@Valid UserDto user, BindingResult result) {
        ...
        if (theUser != null) {
            throw new DolaingException(BizExceptionEnum.USER_ALREADY_REG);
        }
    ...
    public Tip delete(@RequestParam Integer userId) {
        ...
        if (userId.equals(Const.ADMIN_ID)) {
            throw new DolaingException(BizExceptionEnum.CANT_DELETE_ADMIN);
        }
        assertAuth(userId);
    ...
    public Tip freeze(@RequestParam Integer userId) {
        ...
        //不能冻结超级管理员
        if (userId.equals(Const.ADMIN_ID)) {
            throw new DolaingException(BizExceptionEnum.CANT_FREEZE_ADMIN);
        }
        assertAuth(userId);
    ...
    public Tip setRole(@RequestParam("userId") Integer userId, @RequestParam("roleIds") String roleIds) {
        ...
        //不能修改超级管理员
        if (userId.equals(Const.ADMIN_ID)) {
            throw new DolaingException(BizExceptionEnum.CANT_CHANGE_ADMIN);
        }
        assertAuth(userId);
    ...
    public String upload(@RequestPart("file") MultipartFile picture) {
        ...
        try {
            String fileSavePath = dolaingProperties.getFileUploadPath();
            picture.transferTo(new File(fileSavePath + pictureName));
        } catch (Exception e) {
            throw new DolaingException(BizExceptionEnum.UPLOAD_ERROR);
        }
        return pictureName;
    }

...
public class DictSelectorTag extends Tag {
    ...
    public void render() {
        ...
        if (ToolUtil.isEmpty(attrs.get("code"))) {
            throw new DolaingException(BizExceptionEnum.ERROR_CODE_EMPTY);
        }
        ...
        //根据code查询字典数据
        List<Dictionary> list = GlobalData.DICTIONARY_ARR.get(code);
        ...
        if ("radio".equals(type)) {
            list.forEach(obj -> {
                ...
                if (ToolUtil.isNotEmpty(value) && value.equals(obj.getDictValue())) {
                    html.append("checked ");
                }

                html.append("value=\"" + obj.getDictValue() + "\" >" + obj.getDictLabel() + "</label>\r\n");
            });
            ...
            list.forEach(obj -> {
                if (ToolUtil.isNotEmpty(value) && value.equals(obj.getDictValue())) {
                    html.append("<option selected value=\"" + obj.getDictValue() + "\">" + obj.getDictLabel() + "</option>\r\n");
                } else {
                    html.append("<option value=\"" + obj.getDictValue() + "\">" + obj.getDictLabel() + "</option>\r\n");
                }
            });
            html.append("</select>\r\n");
        }
